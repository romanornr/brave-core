...
ContentSetting GetBraveFPContentSettingFromRules(
  ...
  base::Optional<ContentSettingPatternSource> site_fp_rule;
  base::Optional<ContentSettingPatternSource> site_fp_balanced_rule;
  ...
  if (site_fp_balanced_rule &&
      site_fp_balanced_rule->GetContentSetting() == CONTENT_SETTING_BLOCK) {
    // It's balanced mode.
    return CONTENT_SETTING_DEFAULT;
  }

  if (site_fp_rule)
    return site_fp_rule->GetContentSetting();

  if (global_fp_balanced_rule &&
      global_fp_balanced_rule->GetContentSetting() == CONTENT_SETTING_BLOCK) {
    return CONTENT_SETTING_DEFAULT;
  }
